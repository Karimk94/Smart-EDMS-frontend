...
export const Folders: React.FC<FoldersProps> = ({ onFolderClick, t, apiURL }) => {
  const [counts, setCounts] = useState<{ images: number; videos: number; files: number } | null>(null);

  useEffect(() => {
    const fetchCounts = async () => {
      try {
        const response = await fetch(`${apiURL}/media_counts`);
        if (response.ok) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {folders.map((folder) => {
        const count = counts ? counts[folder.id as keyof typeof counts] : null;
